<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="通告标题" prop="tonggaobiaoti">
          <el-input v-model="ruleForm.tonggaobiaoti" 
              placeholder="通告标题" clearable  :readonly="ro.tonggaobiaoti"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="通告标题" prop="tonggaobiaoti">
              <el-input v-model="ruleForm.tonggaobiaoti" 
                placeholder="通告标题" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
            ...
            <el-col :span="24">
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.dingdanyaoqiu" label="订单要求" prop="dingdanyaoqiu">
                    <span>{{ruleForm.dingdanyaoqiu}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.xuqiuneirong = this.ruleForm.xuqiuneirong.replace(reg,'../../../springboot3c707/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xuqiutonggao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xuqiutonggaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
